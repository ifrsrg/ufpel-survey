...
var app = {
    ...
    onDeviceReady: function() {
      ...
      firebase.auth().onAuthStateChanged(function(user) {
        if (user) {
          ...
          if (user != null) {
            ...
            this.controls.userName.innerHTML = name;
            this.controls.emailUser.innerHTML = email;
            this.controls.imgUser.src = photoUrl;
          }
        } else {
      ...
      /* Caixa de Dialogo */
      if (! app.controls.dialog.showModal) {
        dialogPolyfill.registerDialog(myRes.dialog);
      }
      app.controls.showDialogButton.addEventListener('touchend', function() {
        app.controls.dialog.showModal();
      });
      app.controls.dialog.querySelector('.close').addEventListener('touchend', function() {
        this.controls.dialog.close();
      });
      app.controls.dialog.querySelector('#confirm-button').addEventListener('touchend',app.limparTudo.bind(this));
    },
    confirmar: function(){
      if( this.controls.inputMarca.value.length == 0 || this.controls.inputModelo.value.length == 0 ) {
        ...
        var cadastroComSucesso = myStore.cadastrarTrator();
